...
// 4) Cost calculation (MVP)
export function calculateCost(totalSheets, printSides, additions) {
  // Determine price per sheet from tiers
  let pricePerSheet = 0;
  ...
  const baseSheetCost = totalSheets * pricePerSheet;
  const printSurcharge = totalSheets * COST_PER_SIDE_SURCHARGE * (printSides - 1);

  let addMap = {};
  let totalAddCost = 0;
  if (additions.folding) {
    const c = totalSheets * additions.foldLines * ADDITION_COSTS.foldingPerLinePerSheet;
    addMap['Folding'] = c;
    totalAddCost += c;
  }
  if (additions.foiling) {
    const c = totalSheets * ADDITION_COSTS.foilingPerSheet;
    addMap['Foil Stamping'] = c;
    totalAddCost += c;
  }
  if (additions.embossing) {
    const c = totalSheets * ADDITION_COSTS.embossingPerSheet;
    addMap['Embossing/Debossing'] = c;
    totalAddCost += c;
  }
  if (additions.uv) {
  ...
  const totalEstimatedCost = baseSheetCost + printSurcharge + totalAddCost;
  return {
    baseCost: baseSheetCost + printSurcharge,
    additions: addMap,
